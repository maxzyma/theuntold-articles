...
| **站长亲笔基线 vs LLM 生成** | `model` 字段：`human-baseline` = 站长亲笔（untold-001/002）；`openai-codex` 等 = LLM 生成（untold-003/004…） |
| **原始基线副本** | `baselines/` 目录留两篇站长亲笔的原始 markdown，作为 LLM 训练 / 对照评估的语义独立锚点 |
...
|------|------|-------|--------|-----|
| `untold-001` | 吴恩达：最快的团队，人人都是产品经理（站长亲笔基线） | `human-baseline` | 2026-04-28 | [→](https://theuntold.ai/u/untold-001) |
| `untold-002` | Anthropic 产品负责人：从 6 个月到 1 天的发版秘密，harness 会被模型当早餐吃掉（站长亲笔基线） | `human-baseline` | 2026-04-28 | [→](https://theuntold.ai/u/untold-002) |
| `untold-003` | Anthropic 产品负责人：从 6 个月到 1 天的发版秘密，harness 会被模型当早餐吃掉 | `openai-codex` | 2026-05-09 | [→](https://theuntold.ai/u/untold-003) |
| `untold-004` | 吴恩达：最快的团队，人人都是产品经理 | `openai-codex` | 2026-05-09 | [→](https://theuntold.ai/u/untold-004) |

> 「站长亲笔基线」是 LLM 上线前站长手写的两篇批判，作为 prompt 训练样本 + LLM 风格基线 + 现在的天然 LLM-vs-Human 对照样本。

### LLM-vs-Human 主题对照

| 上游文章 | 站长亲笔（baseline） | LLM 生成 | 对照价值 |
|---------|---------------------|----------|---------|
| 吴恩达 _The Batch Issue 349_ | `untold-001` | `untold-004` | 同议题，看 LLM 漏掉了什么 / 加入了什么 |
| Anthropic Mike Krieger（Lenny's Podcast / 20VC） | `untold-002` | `untold-003` | 同上游访谈，看 LLM 风格 vs 站长风格的 deviation |

面向**批判性媒体研究 / LLM 评估**的天然配对数据集——`diff articles/untold-001.md articles/untold-004.md` 即得对照。
...
│   ├── untold-003.md
│   └── untold-004.md
└── baselines/                   # 站长亲笔基线（与 articles/untold-001/002 同源，独立留存为语义锚点）
